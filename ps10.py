...
class Hand(object):
    ...
    def containsLetters(self, letters):
        ...
        for letter in lettersDict.keys():
            if lettersDict[letter] <= self.handDict.get(letter, 0):
                contain = True
            else:
                contain = False
                return contain
    ...
    def isEmpty(self):
        ...
        """
        empty = False
        letterNumList = self.handDict.values()
        if sum(letterNumList) == 0:
            empty = True
        return empty
    ...
    def __eq__(self, other):
        ...
        """
        equal = False
        if self.initialSize == other.initialSize and self.handDict == other.handDict:
            equal = True
        return equal
